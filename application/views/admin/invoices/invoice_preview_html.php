...
<div id="invoice-preview">
    <div class="row">
      ...
      if ($invoice->recurring > 0 || $invoice->is_recurring_from != null) {
        ...
        <!-- Custom code by Michael -->
        <?php
    $CI = &get_instance(); // Get CodeIgniter instance
    
    // Load the invoices_model using $CI
    $CI->load->model('payment_modes_model');
    
    $myinvoice_id = $invoice->id; // Assuming $invoice is available
    
    // Get the allowed payment modes serialized string from the invoice
    $this->db->select('allowed_payment_modes');
    $this->db->from('tblinvoices');
    $this->db->where('id', $myinvoice_id);
    
    $query1 = $this->db->get();
    $invoice_record = $query1->row();
    
    if ($invoice_record) {
        // Unserialize the allowed payment modes
        $allowed_payment_modes = unserialize($invoice_record->allowed_payment_modes);
        
        if ($allowed_payment_modes) {
            $mode_ids = [];
            foreach ($allowed_payment_modes as $mode) {
                $mode_ids[] = $mode;
            }
            
            // Fetch the payment mode names based on the mode IDs
            $this->db->select('name');
            $this->db->from('tblpayment_modes');
            $this->db->where_in('id', $mode_ids);
            
            $query2 = $this->db->get();
            $payment_records = $query2->result();
            
            if ($payment_records) {
                $payment_mode_names = [];
                foreach ($payment_records as $record) {
                    $payment_mode_names[] = $record->name;
                }
                echo "<h5>Allowed payment modes for this invoice: " . implode(", ", $payment_mode_names) . "</h5>";
            } else {
                echo "<h5>Payment Modes Not Found</h5>"; // Handle no payment record case
            }
        } else {
            echo "<h5>No Payment Modes Allowed</h5>"; // Handle no allowed payment modes case
        }
    } else {
        echo "<h5>Invoice Not Found</h5>"; // Handle no invoice record case
    }
?>

        <!-- End of Custom code by Michael -->
        <?php } ?>
