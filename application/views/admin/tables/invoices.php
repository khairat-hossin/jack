...
return App_table::find('invoices')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            'total_tax',
            'total',
            '(SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE ' . db_prefix() . 'invoicepaymentrecords.invoiceid = ' . db_prefix() . 'invoices.id) as amount_paid', // Subquery to get amount paid
            // '(SELECT GROUP_CONCAT(note SEPARATOR ", ") FROM ' . db_prefix() . 'notes WHERE ' . db_prefix() . 'notes.rel_id = ' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'notes.rel_type = "invoice") as notes', // Subquery to get notes
             db_prefix() . 'notes.description as notes',
            // 'YEAR(date) as year',
            'date',
            'duedate',
        ...
        $join = [
            ...
            'LEFT JOIN ' . db_prefix() . 'notes ON ' . db_prefix() . 'notes.rel_id = ' . db_prefix() . 'invoices.id',
        ];
        ...
        foreach ($rResult as $aRow) {
            ...
            //Discount
            $row[] = e(app_format_money($aRow['discount_total'], $aRow['currency_name']));
            ...
            //Amount Due
            $amount_due = floatval($aRow['total']) - floatval($aRow['amount_paid']);
            $row[] = e(app_format_money($amount_due, $aRow['currency_name']));
            //Notes
            $row[] = $aRow['notes'];
            //Year
            // $row[] = e($aRow['year']);
            ...
            // $row[] = render_tags($aRow['tags']); //Not needed
            //Status
