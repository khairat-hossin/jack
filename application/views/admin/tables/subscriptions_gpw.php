...
array_push($where, 'AND ' . db_prefix() . 'subscriptions.type="gpw"');
...
foreach ($rResult as $aRow) {
    ...
    $link       = admin_url('subscriptions_gpw/edit/' . $aRow['id']);
    ...
    if (has_permission('subscriptions', '', 'edit')) {
        $outputName .= ' | <a href="' . admin_url('subscriptions_gpw/edit/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    if ((empty($aRow['stripe_subscription_id'])
        ...
        $outputName .= ' | <a href="' . admin_url('subscriptions_gpw/delete/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    $outputName .= '</div>';
