...
class Subscription_gpw extends ClientsController
{
    public function __construct()
    {
        ...
        $this->load->library('stripe_subscriptions_gpw');
    ...
    public function index($hash)
    {
        ...
        $data['publishableKey'] = $this->stripe_subscriptions_gpw->get_publishable_key();
        $plan                   = $this->stripe_core_gpw->get_plan($subscription->stripe_plan_id);

        check_stripe_subscription_environment($subscription);

        if (!empty($subscription->stripe_subscription_id) && !empty($data['publishableKey'])) {
            $data['stripeSubscription'] = $this->stripe_subscriptions_gpw->get_subscription([
        ...
        $product       = $this->stripe_subscriptions_gpw->get_product($plan->product);

        $upcomingInvoice->lines       = new stdClass();
        $upcomingInvoice->lines->data = [];

        $c = $this->db->query('select * from '.db_prefix().'currencies where isdefault="1"')->row();
        
        $upcomingInvoice->lines->data[] = [
            'description' => $this->lineProductDescription($product, $plan, $c->name),
    ...
    public function subscribe($subscription_hash)
    {
        ...
        $cancelUrl                             = site_url('subscription_gpw/' . $subscription_hash);
        $customerExistsButWithoutPaymentMethod = false;
        if (isset($subscription->stripe_customer_id)) {
            ...
            $customer = $this->stripe_core_gpw->get_customer($stripe_customer_id);
        ...
        $sessionData = [
            ...
            'mode'                 => 'setup',
            'success_url'          => site_url('subscription_gpw/complete_setup/' . $subscription->hash . '?session_id={CHECKOUT_SESSION_ID}'),
    ...
    public function complete_setup($hash)
    {
        ...
        try {
            $session = $this->stripe_core_gpw->retrieve_session([
            ...
            if ($session->client_reference_id) {
                $customer = $this->stripe_core_gpw->get_customer($session->client_reference_id);
                ...
                $customer = $this->stripe_core_gpw->update_customer($session->client_reference_id, $customerPayload);
            } else {
                $customer = $this->stripe_subscriptions_gpw->create_customer($customerPayload);
            }
            
            $payment_method->attach(['customer' => $customer->id]);

            $this->stripe_core_gpw->update_customer($customer->id, [
                'invoice_settings' => [
        ...
        redirect(site_url('subscription_gpw/' . $hash));
    }
    ...
    protected function create_subscription($subscription, $customer_id)
    {
        try {
            ...
            $stripeSubscription = $this->stripe_subscriptions_gpw->subscribe($customer_id, $params);
            ...
            if ($stripeSubscription->status === 'incomplete') {
                ...
                $this->subscriptions_model->update($subscription->id, ['stripe_subscription_id' => $stripeSubscription->id,'status'=> $stripeSubscription->status,]);
        ...
        redirect(site_url('subscription_gpw/' . $subscription->hash));
    }
