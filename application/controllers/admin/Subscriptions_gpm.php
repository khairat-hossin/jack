...
class Subscriptions_gpm extends AdminController
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/subscriptions_gpm/manage', $data);
    }
    ...
    public function create()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('subscriptions_gpm/edit/' . $insert_id));
        }
        ...
        try {
            ...
            $data['capture_charge'] = $this->stripe_subscriptions_gpw->get_capture_charge();*/
            // echo '<pre>'; print_r($data['capture_charge']); exit;
            $data['plans'] = $this->stripe_subscriptions_gpw->get_plans();
            $this->load->library('stripe_core');
            $data['stripe_tax_rates'] = $this->stripe_core->get_tax_rates();
        } catch (Exception $e) {
            if ($this->stripe_subscriptions_gpw->has_api_key()) {
        ...
        $this->load->view('admin/subscriptions_gpm/subscription', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if ($this->input->post()) {
            ...
            try {
                ...
                $this->stripe_subscriptions_gpw->update_subscription($stripeSubscriptionId, $update, $subscription, $prorate);
            } catch (Exception $e) {
                set_alert('warning', $e->getMessage());
                redirect(admin_url('subscriptions_gpm/edit/' . $id));
            }
        ...
        try {
            ...
            if (!empty($subscription->stripe_subscription_id)) {
                $data['stripeSubscription'] = $this->stripe_subscriptions_gpw->get_subscription($subscription->stripe_subscription_id);
                ...
                if ($subscription->status != 'canceled' && $subscription->status !== 'incomplete_expired') {
                    $data['upcoming_invoice'] = $this->stripe_subscriptions_gpw->get_upcoming_invoice($subscription->stripe_subscription_id);
    ...
    public function cancel($id)
    {
        ...
        try {
            ...
            $ends_at = time();
            if ($type == 'immediately') {
                $this->stripe_subscriptions_gpw->cancel($subscription->stripe_subscription_id);
            // The mail sent via the webhook
                ...
                $ends_at = $this->stripe_subscriptions_gpw->cancel_at_end_of_billing_period($subscription->stripe_subscription_id);
            } else {
    ...
    public function sync()
    {
        ...
        $this->load->library('stripe_subscriptions_gpw_synchronizer');

        echo '<a href="' . admin_url('subscriptions_gpm') . '">Go Back</a><br /><br />';

        $this->stripe_subscriptions_gpw_synchronizer->sync();
    }
    ...
    public function resume($id)
    {
        ...
        try {
            $this->stripe_subscriptions_gpw->resume($subscription->stripe_subscription_id, $subscription->stripe_plan_id);
    ...
    public function delete($id)
    {
        ...
        if (strpos($_SERVER['HTTP_REFERER'], 'clients/') !== false) {
            ...
            redirect(admin_url('subscriptions_gpm'));
        }
    }
}