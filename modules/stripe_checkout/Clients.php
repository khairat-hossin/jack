...
class Clients extends ClientsController
{
        ...
        if ($group != 'edit_task') {
            if ($group == 'project_overview') {
                ...
                $data['project_tasks']  = $this->projects_model->get_tasks($id, [
                    ...
                    $note = $this->db->get(db_prefix() . 'taskstimers')->row();
                    $data['task_detail_note'] = $note->note;
    ...
    public function update_credit_card()
    {
        ...
        try {
            ...
            $data['stripe_customer'] = $this->stripe_core_gpw->get_customer($client->stripe_id);
            $type = 'gpm';
        }
        if($type == 'gpw')
        {
    ...
    public function success_update_card()
    {
        ...
        if($type == 'gpm')
        {
           try {
                ...
                // print_r($session);
                // exit();
                $this->stripe_core_gpw->update_customer($session->setup_intent->metadata->customer_id, [
                    'invoice_settings' => [
                        'default_payment_method' => $session->setup_intent->payment_method->id,
                      ],
                  ]);
    ...
    public function credit_card()
    {
        ...
        if (!empty($data['stripe_customer']->invoice_settings->default_payment_method)) {
            ...
            if($type == 'gpw') {
                ...
                $data['payment_method'] = $this->stripe_core_gpw->retrieve_payment_method($data['stripe_customer']->invoice_settings->default_payment_method);
            }
        }
