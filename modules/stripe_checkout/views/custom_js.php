 ...
 if($CI->uri->segment(1) == 'invoice')
 {
    ...
    <script>
       $('document').ready(function()
       {
        ...
        //   $.ajax({
        //      url: "<?php echo base_url();?>stripe_checkout/stripe_checkout_module/subscription_html",
        //      type: "GET",
            ...
            $('#online_payment_form').on('submit',function(event){
              ...
              for(i=0; i < payment.length; i++)
              {
                ...
                if(stripe == 1)
                  {
                      $.ajax({
                        url: "<?php echo base_url();?>stripe_checkout/stripe_checkout_module/card_details_exist",
                        type: "GET",
       ...
       function subscription_change(select)
       {
         var sub = $(select).val();
         $.ajax({
           url: "<?php echo base_url();?>stripe_checkout/stripe_checkout_module/subscription_data",
           type: "GET",
           data : {'sub_id':sub},
           success: function(response)
           {
...
if($CI->uri->uri_string() == 'clients/profile')
{
   ...
   <script>
      $('document').ready(function()
      {
        ...
        $.ajax({
          url: "<?php echo base_url();?>stripe_checkout/stripe_checkout_module/card_detail_html",
          type: "POST",
          datatype: 'html',
...
if($CI->uri->uri_string() == 'products/client/place_order')
{
   ...
   <script>
      $('document').ready(function()
      {
            ...
            {
              $(form1).attr('action',"<?php echo base_url();?>stripe_checkout/stripe_checkout_module/place_order");
            }
      });
   </script>
 <?php } ?>
