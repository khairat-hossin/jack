...
foreach ($rResult as $aRow) {
    ...
    if (has_permission('lead_manager', '', 'can_audio_call') && get_option('call_twilio_active')) {
     ...
     if(isset($callerIdNumber) && !empty($callerIdNumber)){
         $aRow["phonenumber"] = str_replace(".","",str_replace(")","",str_replace("(","",str_replace("-","",str_replace(" ","",$aRow["phonenumber"])))));
     ...
     //ORIGINAL   $allow_call = '<li><a href="javascript:void(0);" onclick="dialPhone('.$aRow["phonenumber"].','.$aRow['id'].','.$callerIdNumber.');"><i class="fa fa-phone" aria-hidden="true" data-toggle="tooltip" data-title="Call"></i></a></li>'; 
   //     $allow_call = '<li class="fa-stack"><a href="SoftphonePro.Call:'.$aRow["phonenumber"].'" title="'._l('lead_manager_twilio_number_not_assigned').'"><i class="fa fa-phone" aria-hidden="true" data-toggle="tooltip" data-title="Call"></i></a></li>'; 
    }else{
