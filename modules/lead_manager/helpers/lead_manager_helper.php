...
function get_lead_id_by_number($number)
{
   if($number){
       $CI = &get_instance();
       $CI->db->select('id');
      $number_format1 = $number;
       $number_format2 = $number;
      // echo $number;
       if(stripos($number, ') ')){
            $numbers_part = explode("-", $number);//(754) 247-2546
            $numbers_part[0] = trim($numbers_part[0], "(");
            $numbers_part[0] = str_replace(") ","-",$numbers_part[0]);
            //$numbers_part[0] = trim($numbers_part[0], ") ");
            $number_format1 = $numbers_part[0].'-'.$numbers_part[1];
            $number_format2 = '+1'.str_replace(") ","",str_replace("-","",$numbers_part[0])).''.$numbers_part[1];
       }else if(substr($number, 0, 2) == '+1'){

            $firstpart = substr($number, 2, 3);
            $secondpart = substr($number, 5, 3);
            $thirdpart = substr($number, 8, 4);
            $number_format1 = $firstpart.'-'.$secondpart.'-'.$thirdpart;
            $number_format2 = '('.$firstpart.') '.$secondpart.'-'.$thirdpart;
        
        }else {
            $numbers_part = explode("-", $number);//754-247-2546
            $number_format1 = '('.$numbers_part[0].') '.$numbers_part[1].'-'.$numbers_part[2];
            $number_format2 = $numbers_part[0].$numbers_part[1].$numbers_part[2];

        }
       //echo $number; exit;
      // $CI->db->where(['phonenumber' => $number]);
       $CI->db->where('phonenumber LIKE "%'. $number . '%"');
       $CI->db->or_where('phonenumber LIKE "%'. $number_format1 . '%"');
       $CI->db->or_where('phonenumber LIKE "%'. $number_format2 . '%"');
       $res= $CI->db->get(db_prefix() . 'leads')->row();
