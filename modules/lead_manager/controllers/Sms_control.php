...
public function incoming_sms_webhook()
{
    ...
    if($this->input->get()){
        ...
        //$leadid = 128;
        //echo $this->input->get('From'); 
        //echo "leadid: ".$leadid; exit;
        if($leadid){
            ...
            $insert['is_client'] = 0;
            $insert['is_read'] = 'no';
            $insert['json_data'] = json_encode(array('data' => $this->input->get(), 'insert' => $insert));
            $CI = &get_instance();
            
            $res= $CI->db->insert(db_prefix() . 'lead_manager_conversation',$insert);
            $leadid = $this->input->get('From');
            $insert['from_id'] = $leadid;
            $insert['to_id'] = $this->input->get('To');
            $CI->db->insert(db_prefix() . 'lead_manager_conversation_backup',$insert);
        }
        if($clientid){
            $insert['msg_sid'] = $this->input->get('SmsMessageSid');
            $insert['from_number'] = $this->input->get('From');
