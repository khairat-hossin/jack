...
function lead_manager_module_init_menu_items() {
	...
	if (has_permission('lead_manager', '', 'view_own') || has_permission('lead_manager', '', 'view')) {
		...
		//	'href'     => admin_url('lead_manager/shedule_appointment'),
		     'href'     => 'https://docs.google.com/spreadsheets/d/1yeojimNPOq5gdxaSq_3fjZQqlA1DcGigpHdtKql5IG4/edit#gid=2012579918',
			'position' => 2,
		]); 
...
function validate_activation_module()
{
	$CI = &get_instance();
	$verified = $CI->app_object_cache->get(MODULE_LEAD_MANAGER.'_is_verified');
	if (!$verified) {
		$verified = get_option(MODULE_LEAD_MANAGER.'_is_verified');
		if (!empty($verified)) {
			$CI->app_object_cache->add(MODULE_LEAD_MANAGER.'_is_verified', $verified);
		}else{
			$CI->load->library(MODULE_LEAD_MANAGER.'/CodeEnv');
			$code_env_resposne = $CI->codeenv->verifyPurchase(MODULE_LEAD_MANAGER, null);
			if (!$code_env_resposne) {
				set_alert('danger', _l('lm_module_not_register_alert'));
...
hooks()->add_action('pre_activate_module', function($module){
	if ($module['system_name'] == MODULE_LEAD_MANAGER) {
		if (!option_exists(MODULE_LEAD_MANAGER.'_is_verified') && empty(get_option(MODULE_LEAD_MANAGER.'_is_verified'))) {
			$CI = & get_instance();
			$data['module_name'] = MODULE_LEAD_MANAGER; 
			$data['title']       = $module['headers']['module_name']. " module activation";
			echo $CI->load->view(MODULE_LEAD_MANAGER.'/admin/validate_module', $data, true);
			exit();
		}
	}
});
