        ...
        if ($type == 'subscription') {
            ...
            try {
                ...
                if ($this->stripe_subscriptions->has_api_key()) {
                    ...
                    $data['product_error'] = _l('api_key_not_set_error_message',
                        '<a href="' . admin_url('settings?group=payment_gateways&tab=online_payments_stripe_tab') . '">Stripe Checkout</a>');
                }
            }
    ...
    public function add($type)
    {
        ...
        if ($type == 'subscription') {
            ...
            if ($this->input->post('related_to') == 'customer_groups') {
                ...
                $data['client_id'] = null;
            } elseif ($this->input->post('related_to') == 'customers') {
            ...
            $data = [
                ...
                'created_from'     => get_staff_user_id(),
                'allowed_payment_modes'     => $this->input->post('allowed_payment_modes') ? serialize($this->input->post('allowed_payment_modes')) : serialize([]),
            ];
