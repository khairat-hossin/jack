    ...
    <div class="panel-body">
        <ul class="nav nav-tabs no-margin " role="tablist">
            <li role="presentation" class="<?= (!$group) ? 'active' : null ?>">
                <a data-group="all_products" href="<?php
                echo site_url('services/'); ?>" role="tab"><i class="fa fa-th" aria-hidden="true"></i> <?php
                    echo _l('all'); ?></a>
            </li>
            <?php
            if (get_option('hide_invoice_subcription_group_from_client_side') != 1) { ?>
                <?php
                if (get_option('enable_subscription_products_view') == 1) { ?>
                    <li role="presentation" class="subscription_products <?= ($group && $group == 'subscriptions') ? 'active' : '' ?>">
                        <a data-group="subscription_products" href="<?php
                        echo site_url('services/category/subscriptions'); ?>" role="tab"> <?php
                            echo _l('subscriptions'); ?></a>
                    </li>
                <?php
                } ?>
                <?php
                if (get_option('enable_invoice_products_view') == 1) { ?>
                    <li role="presentation" class="invoice_products <?= (isset($group) && $group == 'invoices') ? 'active' : '' ?>">
                        <a data-group="invoice_products" href="<?php
                        echo site_url('services/category/invoices'); ?>" role="tab"> <?php
                            echo _l('invoice_product'); ?></a>
                    </li>
                <?php
                } ?>
            <?php
            ...
            foreach ($groups as $category) { ?>
                <?php
                $slug = slug_it($category->name); ?>
                <li role="presentation" class="project_tab_activity <?= (($category->id == $group)) ? 'active' : '' ?>">
                    <a data-group="<?= $slug ?>" href="<?php
                    echo site_url('services/category/' . $category->id . '/' . $slug); ?>" role="tab"><?php
            ...
            <div class="col-md-4 col-sm-6 mbot30 ">
                <div class="card panel-body">
                    <div class="card-body">
                        ...
                        <?php
                        $slug = slug_it($product->name); ?>
                        <?php
                        if (!isset($product->is_recurring)) { // is subscription product
                            ?>
                            <?php
                            if (get_option('enable_products_more_info_button') == 1) { ?>
                                <a href="<?= site_url('services/details/sub/' . $product->id) . '/' . $slug ?>"
                                   class="btn btn-default btn-xs bold mtop10"><?= _l('more_info') ?></a>
                            <?php
                            }
                            $subtext = app_format_money(strcasecmp($product->currency, 'JPY') == 0 ? $product->price : $product->price / 100,
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mright15 mleft20">
                                    <?php
                                    echo render_custom_fields('products'); ?>
                                </div>
                                <div class="col-md-6 col-xs-6">
                                    ...
                                    if (!$showQuantity) {
                                        ...
                                        ?>
                                        <input type="number" name="quantity" min="1" class="form-control" placeholder="<?= _l('quantity') ?>"
                                               required>
                                    <?php
                                    } ?>
                                </div>
                        ...
                        <?php
                            ...
                            <?php
                            if ((get_option('enable_products_more_info_button') == 1)) { ?>
                                <a href="<?= site_url('services/details/inv/' . $product->id) . '/' . $slug ?>"
                                   class="btn btn-default btn-xs bold mtop10"><?= _l('more_info') ?></a>
                            <?php
                            } ?>
                            <h4 class="card-text bold mtop10 "><?= html_entity_decode($subtext) ?></h4>
                                    ...
                                    if (!$showQuantity) {
                                        ...
                                        <div class="form-group">
                                            <input type="number" class="form-control" name="quantity" min="1" id="" value="1"
                                                   placeholder="<?= _l('quantity') ?>" required>
                                        </div>
                                    <?php
                                    } ?>
                                </div>
