...
<?php } ?>
<?php echo form_open('services/products/add/invoice', ['id' => 'stripeProductsForm', 'class' => '_transaction_form']); ?>
    <div class="row">
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            <div class="bg-stripe mbot15">
                ...
                echo render_select('group', $groups, ['id', 'name'], 'product_group', $selected, $s_attrs, [], '', 'ays-ignore');
                ?>
            </div>
            <div class="bg-stripe mbot15">
                <?php
                ...
                <div id="customers">
                    <div class="form-group select-placeholder">
                        ...
                        <select id="clientid" name="clientid" data-live-search="true" data-width="100%"
                                class="ajax-search<?php if (isset($product) && empty($product->clientid)) {
            ...
            $s_attrs = ['multiple' => true, 'data-show-subtext' => true, 'data-none-selected-text' => _l('no_tax')];
            echo render_select('tax[]', $taxes, ['id', 'name', 'taxrate'], 'tax', $selected, $s_attrs, [], '', 'ays-ignore');
            ?>
            <div class="bg-stripe mbot15">
                <?php
                $selected = 0;
                $selected = (isset($product) && ($product->is_recurring == 1)) ? 1 : 0;
                $s_attrs = ['data-none-selected-text' => _l('dropdown_non_selected_tex')];
                $relations = [];
                $relations[] = ['id' => '0', 'name' => _l('no')];
                $relations[] = ['id' => '1', 'name' => _l('yes')];
                echo render_select('is_recurring', $relations, ['id', 'name'], 'is_recurring', $selected, $s_attrs, [], '', 'ays-ignore');
                ?>
                <div id="recurring">
                    <div class="col-md-6">
                        <?php $value = (isset($product) && $product->is_recurring == 1 ? $product->interval : 1); ?>
                        <?php echo render_input('interval', '', $value, 'number', ['min' => 1]); ?>
                    </div>
                    <div class="col-md-6">
                        <select name="interval_type" id="interval_type" class="selectpicker" data-width="100%"
                                data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
