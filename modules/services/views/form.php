...
<?php } ?>
<?php echo form_open('services/products/add/subscription', ['id' => 'stripeProductsForm', 'class' => '_transaction_form']); ?>
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="form-group">
                <div class="checkbox checkbox-primary">
                    <input type="checkbox" id="description_in_item" class="ays-ignore"
                           name="description_in_item" <?php if (isset($product) && $product->description_in_item == '1') {
                        echo ' checked';
                    } ?>>
                    <label for="description_in_item"><i class="fa fa-question-circle" data-toggle="tooltip"
                                                        data-title="<?php echo _l('description_in_invoice_item_help'); ?>"></i> <?php echo _l('description_in_invoice_item'); ?>
                    </label>
                </div>
            </div>
            <?php $value = (isset($product) ? $product->long_description : ''); ?>
            <?php echo render_textarea('long_description',
                _l('long_description') . ' <i class="fa fa-question-circle" data-toggle="tooltip" data-title="' . _l('long_description_hint') . '"></i>',
                $value, [], [], '', 'ays-ignore'); ?>
            <?php $value = (isset($product) ? $product->terms : ''); ?>
            <?php echo render_textarea('terms', 'terms_and_conditions', $value, ['placeholder' => _l('subscriptions_terms_info')], [], '',
                'ays-ignore'); ?>
        ...
        <div class="col-md-6">
            <div class="bg-stripe mbot15">
                ...
                echo render_select('group', $groups, ['id', 'name'], 'group', $selected, $s_attrs, [], '', 'ays-ignore');
                ?>
            </div>
            <div class="bg-stripe mbot15">
                <?php
                ...
                $s_attrs = ['data-show-subtext' => true];
                $relations = [];
                $relations[] = ['id' => 'customers', 'name' => 'customer'];
                $relations[] = ['id' => 'customer_groups', 'name' => _l('customer_group')];
                echo render_select('related_to', $relations, ['id', 'name'], 'related_to', $selected, $s_attrs, [], '', 'ays-ignore');
                ?>
                ...
                <div id="customer_groups">
                    ...
                    $s_attrs = ['data-show-subtext' => true];
                    echo render_select('customer_group', $customers_groups, ['id', 'name'], 'customer_group', $selected, $s_attrs, [], '',
                        'ays-ignore');
                    ?>
                </div>
            </div>
            <div class="bg-stripe mbot15">
                <div class="form-group select-placeholder">
                    ...
                    <select id="stripe_plan_id" name="stripe_plan_id" class="selectpicker" data-live-search="true" data-width="100%"
                        ...
                        <?php if (isset($plans->data)) { ?>
                            <?php foreach ($plans->data as $plan) {
                                ...
                                $subtext = app_format_money(strcasecmp($plan->currency, 'JPY') == 0 ? $plan->amount : $plan->amount / 100,
                                ...
                                <option value="<?php echo html_escape($plan->id); ?>"
                                        data-interval-count="<?php echo html_escape($plan->interval_count); ?>"
                                        data-interval="<?php echo html_escape($plan->interval); ?>"
                                        data-amount="<?php echo html_escape($plan->amount); ?>"
                                        data-subtext="<?php echo html_escape($subtext); ?>" <?php echo html_escape($selected); ?>>
                                    <?php
            ...
            <div class="form-group select-placeholder">
                ...
                <select class="selectpicker" data-width="100%" name="stripe_tax_id" data-none-selected-text="<?php echo _l('no_tax'); ?>">
                    ...
                    <?php foreach ($stripe_tax_rates->data as $tax) {
                        ...
                        ?>
                        <option value="<?php echo html_escape($tax->id); ?>"
                                data-subtext="<?php echo html_escape($tax->display_name); ?>" <?php if (isset($product) && $product->stripe_tax_id == $tax->id) {
                            echo ' selected';
                        } ?>><?php echo html_escape($tax->percentage); ?>%
                        </option>
                    <?php } ?>
                </select>
            </div>
