...
if (!$CI->db->field_exists('stripe_connect', db_prefix() . 'clients')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'clients`
      ADD COLUMN `stripe_connect` LONGTEXT  DEFAULT "gp_marketing" NULL');
}
if (!$CI->db->field_exists('related_to', db_prefix() . 'invoices')) {
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'invoices`
        ADD `related_to` text NULL');
}