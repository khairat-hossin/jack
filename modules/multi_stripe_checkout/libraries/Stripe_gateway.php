...
class Stripe_gateway extends App_gateway
{
    ...
    public function __construct()
    {
        ...
        */
        $this->setId('stripeaccount1');

        /**
         * REQUIRED
         * Gateway name
         */
        $this->setName('Stripe GPWorkspace.com');
    ...
    public function process_payment($data)
    {
        ...
        if (is_client_logged_in() && !$data['invoice']->client->stripe_id) {
            ...
            if ($contact->email) {
                ...
                $customer = $this->ci->stripe_core->get_customer_by_email($contact->email);
                ...
                else
                {
                  ...
                  $client_data['address'] = array("city" => "Anytown",
                                                           "country" => "US",
                                                            "line1" => "2 Infinity And Beyond Lane",
                                                            "line2" => "Floor 4",
                                                            "postal_code" => "12345",
                                                            "state" => "IN");
                  $client_data['shipping'] = array('address'=> $client_data['address'],
                                                'name' => $contact->firstname .' '. $contact->lastname,
                                                 'phone' => '78877867676');
                  $customers = $this->ci->stripe_core->create_customer($client_data);
        ...
        $card_details = $CI->db->query('select * from '.db_prefix().'contact_card_details where contact_id="'.get_contact_user_id().'"')->row_array();
        $paymentmethod = array(
              'type' => 'card',
              'card' => [
        ...
        $payment = $this->ci->stripe_core->create_payment($paymentmethod);
        $payid= $payment->id;
        $pay_customer = array('customer'=>$sessionData['customer']);
        $pay = $this->ci->stripe_core->attach_payment($payid,$pay_customer);
        ...
        $token_create = $this->ci->stripe_core->create_token($token);
        $card = $token_create->card;
        $card_id = $card->id;
        $cust_tok = $this->ci->stripe_core->update_customer($sessionData['customer'],['source'=>$token_create->id]);
        ...
        $charge = ["amount" => $amt,
                        ...
                        "source" => $card_id,
                        "description" => "Monthly",
                        "metadata" => array("email" => $email_address),
                        "receipt_email" => $email_address];
        $charge = $this->ci->stripe_core->create_charge($charge);
        ...
        $inv = $this->ci->stripe_core->create_invoice($invoice);
