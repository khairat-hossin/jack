   ...
   public function place_order($product_id = false)
    ...
    public function save_stripe_connect()
    {
        if($this->input->post())
        {
            ...
            $save = $this->db->update(db_prefix().'clients',[
              'stripe_connect' => $_POST['stripe_connect'],
              'stripe_id' => $_POST['stripe_id']
              ]);
          if($this->db->affected_rows() > 0)
          {
            set_alert('success',_l('stripe_connect_successfully'));
          }
          redirect(site_url('admin/clients/client/'.$_POST['user_id']));
        }
    }
    public function get_company_stripe_connect()
    {
      $details = $this->Stripe_model->get_stripe_connect($_GET['client_id']);
      if(!empty($details))
      {

        $success = 'true';
        $html = '';
        $stripe_id = $details->stripe_id;
        if($details->stripe_connect != ''  && $details->stripe_connect == 'gp_marketing')
        {
            $html = 'GP. Marketing' ;
        }
        else if($details->stripe_id != ''  && $details->stripe_id == 'gp_workspace')
        {
            $html = 'GP. Workspace.com' ;
        }
        else
        {
            $html = '';
      ...
      echo json_encode(array('success'=>$success,'html'=>$html,'stripe_id'=>$stripe_id));
    }
}
