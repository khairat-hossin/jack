...
if ($this->ci->input->post('staff_filter')) {
	$where_staff = '';
	$staff_filter = $this->ci->input->post('staff_filter');
	foreach ($staff_filter as $staff) {
		if ($staff != '') {
			if ($where_staff == '') {
				$where_staff = ' AND (staff_id IN (SELECT staffid FROM ' . db_prefix() . 'staff WHERE staffid = ' . $staff . ')';

			} else {
				$where_staff .= 'OR staff_id IN (SELECT staffid FROM ' . db_prefix() . 'staff WHERE staffid = ' . $staff . ')';
			}
		}
	}
	if ($where_staff != '') {
...
if ($this->ci->input->post('date_f')) {
    array_push($where, "AND DATE(start_time) >= '" . to_sql_date($this->ci->input->post('date_f')) . "'");
}
if ($this->ci->input->post('date_t')) {
    array_push($where, "AND DATE(end_time) <= '" . to_sql_date($this->ci->input->post('date_t')) . "'");
}
