...
if ($this->ci->input->post('staff_filter_ats')) {
	$where_staff = '';
	$staff_filter = $this->ci->input->post('staff_filter_ats');
	foreach ($staff_filter as $staff) {
		if ($staff != '') {
			if ($where_staff == '') {
				$where_staff = ' AND (creator IN (SELECT creator FROM ' . db_prefix() . 'timesheets_additional_timesheet WHERE creator = ' . $staff . ')';

			} else {
				$where_staff .= 'OR creator IN (SELECT creator FROM ' . db_prefix() . 'timesheets_additional_timesheet WHERE creator = ' . $staff . ')';
			}
		}
	}
	if ($where_staff != '') {
...
if ($this->ci->input->post('date_f_ats')) {
    array_push($where, "AND DATE(additional_day) >= '" . to_sql_date($this->ci->input->post('date_f_ats')) . "'");
}
if ($this->ci->input->post('date_t_ats')) {
    array_push($where, "AND DATE(additional_day) <= '" . to_sql_date($this->ci->input->post('date_t_ats')) . "'");
}
