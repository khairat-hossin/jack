 ...
 */
	...
	<div class="modal-footer">';
		...
		if ($additional_timesheets->status == 0 && ($check_approve_status == false || $check_approve_status == 'reject')) {
            ...
            $all_departments = $this->departments_model->get();
            $has_HR_role = false;
            foreach($all_departments as $index => $value){
                if( in_array( $value['departmentid'] ,$staff_deparments ) && $value['name'] == "HR Team" ) {
                    $has_HR_role = true;
                }
            }
            // echo '<pre>'; print_r($staff_deparments);echo '<br/>';
            // echo '<pre>'; print_r($all_departments); exit;
			
			if ($check != 'choose' && (is_admin() || $has_HR_role)) {
			...
			if ($this->input->post()) {
				...
				if ($data_timesheet) {
					foreach ($rResult as $aRow) {
						...
						$row[] = ($total_hours > 0) ? (float) number_format($total_hours, 2) : 0;
