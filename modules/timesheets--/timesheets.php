...
function timesheets_module_init_menu_items() {
	$CI = &get_instance();
	$allow_setting_to_show = false;
	$CI->load->model('departments_model');
	$CI->load->model('staff_model');
	$staff_roles = $CI->roles_model->get();
	$id = get_staff_user_id();

	$member = $CI->staff_model->get($id);
	foreach ($staff_roles as $staff_key => $staff_value) {
		if(($staff_value['roleid'] == $member->role) && ($staff_value['name'] == 'Finance Team' || $staff_value['name'] == 'HR Team')){
			$allow_setting_to_show = true;
		}
	}
	if (has_permission('attendance_management', '', 'view_own') ||
		...
		if (is_admin() || $allow_setting_to_show) {
			$CI->app_menu->add_sidebar_children_item('timesheets', [
