    ...
    public function applicable_staff_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = _d($aRow['to_date']);
                if($aRow['is_client'] == 1){
                    $options = icon_btn('commission/delete_applicable_client/' . $aRow['applicable_staff_id'], 'remove', 'fa fa-times btn-danger', ['title' => _l('delete')]);
                }else{
                    $options = icon_btn('commission/delete_applicable_staff/' . $aRow['applicable_staff_id'], 'remove', 'fa fa-times btn-danger', ['title' => _l('delete')]);
                }
