                    ...
                    if (response.length) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i].id)) {
                                ...
                                let conversation_code = this.getListCode(item, (status == 0 || status == 2) && !SBF.isAgent(item.user_type) ? 'new' : null);
                                ...
                                if (active_conversation) {
                                    ...
                                    if (conversation) {
                                        ...
                                        conversations[conversation_index].status_code = status;
                                ...
                                if (!SBChat.tab_active && item.status_code == 2 && (!SBF.isAgent(item.user_type) || payload.preview) && (!SBF.null(item.message) || !SBF.null(item.attachments) || payload.preview)) {
        ...
        getListCode: function (conversation, status) {
            if (conversation instanceof SBConversation) {
                conversation = {
                    ...
                    status_code: conversation.status_code,
                    ...
                    last_update_time: conversation.get('last_update_time'),
            ...
            return `<li data-user-id="${conversation.user_id}" data-conversation-id="${conversation.id}" data-conversation-status="${status}"${!SBF.null(conversation.source) ? ` data-conversation-source="${conversation.conversation_source}"` : ''}>${label_new}<div class="sb-profile"><img loading="lazy" src="${conversation.profile_image}"><span class="sb-name${is_title ? ' sb-custom-name' : ''}">${is_title ? conversation.title : conversation.first_name + ' ' + conversation.last_name}</span><span class="sb-time">${SBF.beautifyTime(conversation.last_update_time)}</span></div><p>${strip.strip(message)}</p></li>`;
        },
                            ...
                            if ([0, 3, 4].includes(status_code)) {
                                for (var i = 0; i < conversations.length; i++) {
                                    if (conversations[i].id == conversation.id) {
                                        conversations[i]['status_code'] = status_code;
                                        break;
                                    }
                                }
                            }
